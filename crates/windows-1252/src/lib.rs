...
#[derive(Debug, Error)]
#[error("unable to encode character {0} as windows-1252")]
pub struct EncodeWin1252Error(char);
...
pub fn decode(byte: u8) -> char {
    if byte.is_ascii() {
        ...
        let index = byte - 0x80;
        let ch = TABLE[index as usize] as u32;
        unsafe { char::from_u32_unchecked(ch) }
    }
}

#[inline]
pub fn encode(ch: char) -> Result<u8, EncodeWin1252Error> {
    if ch.is_ascii() {
        Ok(ch as u8)
    } else {
        let code_point = ch as u32;
        let code_point: u16 = code_point.try_into().map_err(|_| EncodeWin1252Error(ch))?;
        let index = TABLE
            .iter()
            .position(|ch| *ch == code_point)
            .ok_or(EncodeWin1252Error(ch))?;
        Ok(index as u8 + 0x80)
    }
}
...
pub fn decode_string(bytes: Vec<u8>) -> String {
    if bytes.is_ascii() {
        ...
        bytes.into_iter().map(decode).collect()
    }
}

#[inline]
pub fn encode_string(s: String) -> Result<Vec<u8>, EncodeWin1252Error> {
    if s.is_ascii() {
        Ok(s.into_bytes())
    } else {
